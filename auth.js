...
if (document.getElementById('registerForm')) {
    document.getElementById('registerForm').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            // For production: Replace with your EmailJS credentials
...
Climate Smart Solutions`
                    ...
                    // IONOS SMTP Configuration from EmailJS Dashboard
                    await emailjs.send(
                        'service_x3phsl7',        // IONOS SMTP Service ID
                        'template_icqfar5',       // E-Mail-Verifizierung Template ID
                        templateParams,
                        'ZOprGu7EjDZmGl4ql'      // ✅ EmailJS Public Key (Account → General)
                    );

                    console.log('✅ Email sent successfully via EmailJS');
                } else {
                    console.warn('⚠️ EmailJS not loaded. Email will not be sent.');
            ...
            toast.success(
                ...
                `Ein Bestätigungscode wurde an ${email} gesendet. Bitte überprüfen Sie Ihre E-Mails.`,
                3000
            );

            // Redirect to verification page after toast
            setTimeout(() => {
...
function showError(message) {
    ...
    if (typeof toast !== 'undefined') {
        toast.error('Fehler', message, 5000);
    } else {
        // Fallback to old error element method
