...
(() => {
  ...
  async function markSeenBackend() {
    try {
      ...
      // Requires api token (cssberlin_token) — ok if it fails.
  ...
  function showModal(user, opts) {
    ...
    function closeAndRemember(goShop) {
      ...
      if (goShop) {
        // If not on homepage, go there; otherwise gently scroll.
  ...
  async function maybeShowWelcome() {
    ...
    // If backend knows it's already seen, respect it.
    ...
    try {
      if (window.api && typeof window.api.getCurrentUser === 'function') {
        ...
        backendSaysEligible = me?.member_discount_active !== false; // default true
      }
    } catch {
    ...
    // Gentle delay so it feels intentional (and avoids racing with header injection).
